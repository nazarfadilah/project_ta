    ...
    <div class="card border-0 shadow-sm rounded-3">
        <div class="card-header d-flex align-items-center justify-content-between" style="background-color: #C9A961; color: #fff; border-radius: 8px 8px 0 0; padding: 14px 20px;">
            <h6 class="mb-0 fw-semibold" style="font-size: 15px;">
                <i class="fas fa-image me-2"></i>Daftar Gambar Landing
            </h6>
            <a href="{{ route('main.landing.gambar.create') }}" class="btn btn-sm btn-light" style="font-size: 13px; padding: 6px 12px;">
                <i class="fas fa-plus me-1"></i> Tambah Data
            </a>
        </div>
        <div class="card-body" style="padding: 20px;">
            <div class="table-responsive">
                <table id="gambarTable" class="table table-hover table-bordered align-middle" style="width: 100%; font-size: 14px;">
                    ...
                    <tbody>
                        @foreach($gambarLanding as $index => $item)
                        <tr>
                            <td style="text-align: center;">{{ $index + 1 }}</td>
                            <td style="text-align: center;"><span class="badge bg-info">{{ $item->posisi }}</span></td>
                            <td style="text-align: center;">
                                @if($item->path)
                                    <img src="{{ asset('storage/' . $item->path) }}" 
                                         alt="Gambar Landing" 
                                         style="max-width: 120px; max-height: 80px; border-radius: 4px;">
                                @else
                                    <span class="text-muted" style="font-size: 12px;">Tidak ada gambar</span>
                                @endif
                            </td>
                            <td>{{ $item->waktu_upload ? $item->waktu_upload->format('d-m-Y H:i') : '-' }}</td>
                            <td style="text-align: center;">
                                <a href="{{ route('main.landing.gambar.edit', $item->id) }}" 
                                ...
                                <button class="btn btn-sm btn-danger" 
                                        title="Delete"
                                        onclick="hapusData('{{ route('main.landing.gambar.destroy', $item->id) }}')"
                                        style="padding: 4px 10px; font-size: 13px;">
